...
const Timeline: React.FC<TimelineProps> = ({ experiences }) => {
  const sectionOrder: ExperienceInterface["section"][] = [
    "Research Experience",
    "Work Experience",
    "Teaching & Mentoring",
  ];

  const sortByDate = (items: ExperienceInterface[]) => [...items].sort((a, b) => {
  ...
  return (
    <div className="space-y-10">
      {sectionOrder.map((section) => {
        const sectionExperiences = sortByDate(
          experiences.filter((experience) => experience.section === section)
        );

        if (!sectionExperiences.length) {
          return null;
        }
